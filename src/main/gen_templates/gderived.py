...
DERIVED_HEADER = """\
/* Generated file on """ + datetime.now().strftime("%m/%d/%Y, %H:%M:%S") +""", do not modify.  /gen_templates/gderived.py. */
package %s;
...
class Gen:
    ...
    def get_aux(self, name):
        if self.auxiliary is None:
            ...
            directives.execute(directives.load(os.path.join(scriptdir,'template_files', 'gderived-defs')), aux_gen)
    ...
    def dire_incl(self, name, parm, body):
        ...
        def load():
            for d in directives.load(os.path.join(os.path.dirname(os.path.abspath(scriptdir)),'gen_templates','template_files',parm.strip()+'.derived')):
